...
class MasterMind:
    def __init__(self, root):
        ...
        for i in self.colors:
            ...
            n += 1
            c += 1
        self.palette[0].config(
            command=lambda: self.guess(root, self.palette[0]["bg"], obj)
        )  # binding function to palette
        self.palette[1].config(
            command=lambda: self.guess(root, self.palette[1]["bg"], obj)
        )
        self.palette[2].config(
            command=lambda: self.guess(root, self.palette[2]["bg"], obj)
        )
        self.palette[3].config(
            command=lambda: self.guess(root, self.palette[3]["bg"], obj)
        )
        self.palette[4].config(
            command=lambda: self.guess(root, self.palette[4]["bg"], obj)
        )
        self.palette[5].config(
            command=lambda: self.guess(root, self.palette[5]["bg"], obj)
        )
        self.palette[6].config(
            command=lambda: self.guess(root, self.palette[6]["bg"], obj)
        )
        self.palette[7].config(
            command=lambda: self.guess(root, self.palette[7]["bg"], obj)
        )
    ...
    def guess(self, root, choice, obj):
        ...
        if cro != 1:
            self.user_choice.append(
                Button(root, bg=choice, height=1, width=5, relief=RAISED)
            )
